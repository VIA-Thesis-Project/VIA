...
7. **No bulk endpoint for full evaluation presentation.** Status, result, evidence, limitations, knowledge, and recommendations are separate resources and should be cached independently.
8. **`completed_crop_count` is outcome-count based today.** With more than one water regime it is not a unique-crop progress counter. Use `requested_execution_count` / `completed_execution_count` for progress UI.
