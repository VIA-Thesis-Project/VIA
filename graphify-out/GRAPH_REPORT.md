...
## Corpus Check
- 293 files · ~137,185 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2722 nodes · 6845 edges · 193 communities (115 shown, 28 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 857 edges (avg confidence: 0.91)
...
## Communities (193 total, 28 thin omitted)
...
Cohesion: 0.07
Nodes (37): datetime, Exception, Synchronous application orchestration for persisted evaluations., _to_common_support(), _to_comparable_crop(), _to_outcome(), ICropComparisonEngine, Compare durable crop outputs using the authoritative scientific engine. (+29 more)
...
Cohesion: 0.06
Nodes (28): CommonSupport, Validate deterministic scientific ranking semantics., Spatial support shared by the usable crop suitability rasters., validate_comparable_crops(), InvalidEvaluationTransitionError, Raised when an Evaluation lifecycle transition is not allowed., Agroclimatic Evaluation domain layer., Evaluation (+20 more)

### Community 37 - "EnvironmentalInputManifest"
Cohesion: 0.06
Nodes (66): EnvironmentalInputResolutionError, RuntimeError, Raised when an exact caller-selected environmental version cannot be resolved., EnvironmentalInputIntegrityError, Raised when resolved environmental provenance does not match scientific inputs., EnvironmentalInputManifest, EnvironmentalInputSnapshot, Immutable set of exact environmental inputs resolved for an evaluation. (+58 more)
...
Cohesion: 0.09
Nodes (33): Commands expressing Agroclimatic Evaluation use-case intent., EvaluationResultAvailability, StrEnum, Whether persisted outcomes are pending, partial, or final., Explicit fail-only recovery for abandoned evaluation executions., CommonSupportStatus, StrEnum, Evaluation-level scientific common-support result. (+25 more)
...
### Community 45 - "InMemoryEvaluationRepository"
Cohesion: 0.20
Nodes (17): AgroclimaticEvaluationWorker, Logger, Discover queued IDs and delegate all execution semantics to Application., InMemoryEvaluationRepository, _engine_result(), _executor(), FakeEngine, test_claim_conflict_is_benign_and_later_work_continues() (+9 more)
...
### Community 64 - "read_crop_parameterizations_files"
Cohesion: 0.15
...
Cohesion: 0.14
Nodes (33): ComparableCropEvidence, A provider-produced crop mean and rank over common valid support., _common_support(), _decision_evidence(), _domain_common_support(), _evaluate(), _finalized_result(), Any (+25 more)

### Community 67 - "DomainValidationError"
Cohesion: 0.09
Nodes (35): InvalidSpatialInputError, CoverageComputation, Protocol, RuntimeError, ValueError, Application ports for Environmental Information spatial collaboration., Raised when supplied parcel geometry is not topologically usable., Raised when the configured spatial implementation cannot execute. (+27 more)
...
Cohesion: 0.16
Nodes (11): datetime, Environmental Information domain layer., Dataset, Environmental Information domain model., Stable logical identity for a geoenvironmental dataset., _validate_text(), DatasetRepository, DatasetVersionRepository (+3 more)

### Community 75 - "DatasetVersion"
Cohesion: 0.06
Nodes (40): DatasetVersionConflictError, RuntimeError, Domain errors raised by Environmental Information invariants., Raised when a dataset version identifier has already been registered., DatasetVersion, Immutable reproducibility metadata for one dataset release., Positive horizontal and vertical source-cell resolution., SpatialResolution (+32 more)
...
Cohesion: 0.13
Nodes (29): Fail one operator-confirmed orphaned active evaluation., RecoverEvaluation, _artifact(), _evaluation(), _evaluation_in_status(), FakeComparisonEngine, FakeEnvironmentalInformation, _manifest() (+21 more)
...
Cohesion: 0.09
Nodes (32): _is_finite_number(), datetime, Immutable environmental input snapshots owned by Agroclimatic Evaluation., _validate_aware_datetime(), _validate_crs(), _validate_positive_number(), _validate_text(), DomainValidationError (+24 more)
...
### Community 152 - "AgroclimaticEvaluationRecoveryService"
Cohesion: 0.19
...
Cohesion: 0.13
Nodes (18): Settings needed by the current backend composition root., Settings for the PostgreSQL polling worker process., Settings, WorkerSettings, MonkeyPatch, parametrize, Tests for environment-driven application composition settings., test_database_url_selects_postgresql_by_default() (+10 more)

### Community 158 - "create_router"
Cohesion: 0.18
Nodes (17): EnvironmentalInputReferenceInput, ParcelSnapshotInput, Transport-neutral parcel state supplied by an authorized caller., RequestEvaluation, create_router(), get_evaluation(), get_evaluation_evidence(), get_evaluation_result() (+9 more)
...
### Community 161 - "CropComparisonExecutionError"
Cohesion: 0.24
...
Cohesion: 0.06
Nodes (94): CropExecutionStatus, CropSuitabilityEngineError, CropSuitabilityExecutionError, CropSuitabilityRequest, CropSuitabilityResult, ICropSuitabilityEngine, IEnvironmentalInputIntegrityVerifier, InvalidEngineOutputError (+86 more)
...
Cohesion: 0.32
Nodes (16): CropSuiteComparisonAdapter, Compare persisted CropSuiteLite rasters on common valid support., _artifact(), Any, Path, _report(), _request(), StubComparisonRunner (+8 more)
...
Cohesion: 0.32
Nodes (5): _environment_float(), _environment_integer(), _optional_path(), Path, Environment-backed configuration for the VIA application host.

### Community 180 - "via_backend/worker.py"
Cohesion: 0.21
Nodes (10): ArgumentParser, Agroclimatic Evaluation infrastructure layer., load_configured_environmental_input_integrity_verifier(), Build the configured verifier from one deployment binding file., create_worker(), _parser(), _positive_integer(), Separate PostgreSQL polling-worker process and operator recovery CLI. (+2 more)
...
Cohesion: 0.09
Nodes (42): Agroclimatic Evaluation application layer., FinalizedCommonSupport, FinalizedCommonSupportStatus, FinalizedComparableCrop, FinalizedCropOutcome, FinalizedCropOutcomeStatus, FinalizedEvaluationResult, FinalizedEvaluationResultReader (+34 more)
...
### Community 187 - "test_farm_management_domain.py"
Cohesion: 0.40
...
### Community 191 - "ScientificArtifactRole"
Cohesion: 0.20
Nodes (9): _comparison_request(), CropComparisonInput, StrEnum, One durable crop suitability raster offered to scientific comparison., Scientific artifact roles understood by VIA., ScientificArtifactRole, scientific, Opt-in smoke test for the real CropSuiteLite comparison boundary. (+1 more)
...
- **Why does `EnvironmentalInformationService` connect `EnvironmentalInformationService` to `DomainValidationError`, `GetPublishedDatasetVersion`, `Dataset`, `DatasetVersion`, `SpatialExtent`, `CoverageMeasurement`, `via_backend/worker.py`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `create_app()` connect `DatasetVersion` to `FarmManagementService`, `create_database`, `test_agroclimatic_evaluation_api.py`, `Project`, `PostgreSQLEvaluationRepository`, `Parcel`, `EnvironmentalInformationService`, `InMemoryEvaluationRepository`, `agroclimatic_evaluation/application/__init__.py`, `test_farm_management_api.py`, `test_environmental_information_api.py`, `WorkerSettings`, `create_router`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `PostgreSQLEvaluationRepository` connect `PostgreSQLEvaluationRepository` to `Evaluation`, `agroclimatic_evaluation/infrastructure/postgresql_repositories.py`, `EvaluationStatus`, `DatasetVersion`, `via_backend/worker.py`, `AgroclimaticEvaluationRecoveryService`?**
