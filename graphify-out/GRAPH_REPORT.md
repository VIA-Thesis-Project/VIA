# Graph Report - poc_via_cslite  (2026-09-13)

## Corpus Check
- 252 files · ~107,956 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1912 nodes · 4124 edges · 169 communities (95 shown, 31 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 434 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8722b249`
...
1. `Evaluation` - 48 edges
2. `EnvironmentalInformationService` - 43 edges
3. `DatasetVersion` - 43 edges
4. `Parcel` - 38 edges
5. `Dataset` - 34 edges
6. `FarmManagementService` - 34 edges
7. `InMemoryEvaluationRepository` - 33 edges
8. `DomainValidationError` - 30 edges
9. `EvaluationStatus` - 29 edges
10. `DomainValidationError` - 28 edges
...
Cohesion: 0.12
Nodes (25): DomainValidationError, ValueError, Domain errors raised by Farm Management invariants., Raised when a Farm Management value violates a domain invariant., ParcelGeometry, _parse_multi_polygon(), _parse_polygon(), _parse_position() (+17 more)
...
Cohesion: 0.11
Nodes (36): AgroclimaticEvaluationExecutionService, Execute requested crops sequentially through the Application-owned port., AgroclimaticEvaluationWorker, Discover queued IDs and delegate all execution semantics to Application., EvaluationStatus, StrEnum, Architecture-approved lifecycle vocabulary., InMemoryEvaluationRepository (+28 more)

### Community 10 - "CropSuite.py"
Cohesion: 0.10
Nodes (42): # NOTE: self.extent is modified here to align with grid, get_id_list_start(), Get a list of keys from a dictionary that start with a specified prefix.…, aggregate_soil_raster_lst(), calcification_map(), cropsuitability(), get_soil_data(), get_suitability_val_dict() (+34 more)
...
### Community 12 - "Parcel"
Cohesion: 0.17
Nodes (12): ParcelVersionConflictError, RuntimeError, Raised when persisted parcel history changed before a revision was saved., Parcel, A named parcel whose geometry changes only by appending versions., InMemoryParcelRepository, UUID, In-memory Farm Management repository adapters. (+4 more)
...
### Community 15 - "test_environmental_information_api.py"
Cohesion: 0.27
...
Cohesion: 0.20
Nodes (5): process_precday_interp(), Resample mm/day without mixing missing coverage into coastal rainfall. Missing…, write_to_netcdf(), Missing source coverage must neither dilute rainfall nor gain rainfall., PrecipitationUnitsTest
...
Cohesion: 0.06
Nodes (78): CropExecutionStatus, CropSuitabilityEngineError, CropSuitabilityExecutionError, CropSuitabilityResult, InvalidEngineOutputError, RuntimeError, StrEnum, Application-owned boundary for one crop suitability evaluation. (+70 more)
...
### Community 23 - "merge_geotiff.py"
Cohesion: 0.43
Nodes (6): create_cog_from_geotiff(), geotiff_to_smallest_datatype(), Convert image to COG., merge_outputs_no_overlap(), merge_netcdf_files(), Merge multiple NetCDF files based on latitude and longitude coordinates…
...
Cohesion: 0.24
Nodes (13): Multicrop Parcel Evaluation, run_evaluation Service, ADR-004 CropSuiteLite Port and Adapter, Background worker execution, CropSuiteLite isolation boundary, FastAPI, Python, Python and FastAPI backend decision (+5 more)

### Community 31 - "test_environmental_coverage.py"
Cohesion: 0.10
Nodes (18): calculate_slope(), process_tempday_interp(), get_formula(), get_id_list_start(), get_plant_param_interp_forms_dict(), print_crop_param_output(), print_sections(), Reads and parses crop parameterization files from a specified folder path.… (+10 more)
...
Cohesion: 0.08
Nodes (13): Exception, datetime, UUID, InvalidEvaluationTransitionError, RuntimeError, Raised when an Evaluation lifecycle transition is not allowed., Evaluation, An immutable multicrop evaluation and its scientific outcomes. (+5 more)

### Community 38 - "via_backend/worker.py"
Cohesion: 0.15
Nodes (17): ArgumentParser, create_database(), Engine, SessionFactory, Create the shared engine and short-lived session factory., Shared technical infrastructure used by the application composition root., create_worker(), main() (+9 more)
...
### Community 63 - "agroclimatic_evaluation/application/__init__.py"
Cohesion: 0.07
Nodes (53): ExecuteEvaluation, ParcelSnapshotInput, Commands expressing Agroclimatic Evaluation use-case intent., Transport-neutral parcel state supplied by an authorized caller., Request synchronous execution of one already-persisted evaluation., Fail one operator-confirmed orphaned active evaluation., RecoverEvaluation, RequestEvaluation (+45 more)

### Community 64 - "DomainValidationError"
Cohesion: 0.12
Nodes (25): DomainValidationError, ValueError, Raised when evaluation data violates a domain invariant., ParcelSnapshot, _parse_multi_polygon(), _parse_polygon(), _parse_position(), _parse_ring() (+17 more)

### Community 65 - "agroclimatic_evaluation/infrastructure/postgresql_repositories.py"
Cohesion: 0.11
Nodes (26): EvaluationConflictError, Raised when an evaluation identity already exists., CropOutcome, One durable result associated with an Evaluation and requested crop., Base, DeclarativeBase, SQLAlchemy metadata owned by Agroclimatic Evaluation Infrastructure., Declarative base for Agroclimatic Evaluation persistence records. (+18 more)

### Community 67 - "coverage.py"
Cohesion: 0.11
Nodes (27): InvalidSpatialInputError, RuntimeError, ValueError, Application ports for Environmental Information spatial collaboration., Raised when supplied parcel geometry is not topologically usable., Raised when the configured spatial implementation cannot execute., SpatialCoverageUnavailableError, CoverageGeometry (+19 more)
...
Cohesion: 0.11
Nodes (25): DomainValidationError, ValueError, Domain errors raised by Environmental Information invariants., Raised when environmental metadata violates a domain invariant., Environmental Information domain layer., Environmental Information domain model., _validate_text(), Framework-neutral spatial metadata value objects. (+17 more)

### Community 75 - "Dataset"
Cohesion: 0.09
Nodes (26): DatasetVersionConflictError, RuntimeError, Raised when a dataset version identifier has already been registered., Dataset, Stable logical identity for a geoenvironmental dataset., Base, DeclarativeBase, SQLAlchemy metadata owned by Environmental Information Infrastructure. (+18 more)
...
Cohesion: 0.16
Nodes (9): CoverageComputation, DatasetVersion, Immutable reproducibility metadata for one dataset release., DatasetRepository, DatasetVersionRepository, Protocol, UUID, Repository abstractions for Environmental Information aggregates. (+1 more)

### Community 136 - "Project"
Cohesion: 0.10
Nodes (13): Project, An agricultural project that groups parcels., ParcelRepository, ProjectRepository, Protocol, UUID, Repository abstractions for Farm Management aggregates., PostgreSQLProjectRepository (+5 more)

### Community 137 - "test_agroclimatic_evaluation_postgresql.py"
Cohesion: 0.11
Nodes (37): ValueError, Safety guard shared by destructive PostgreSQL/PostGIS integration tests., Raised before destructive tests target a database that is not explicitly safe., Return a safe integration-test URL without ever including it in errors., Read and validate the test-only database settings before any DB operation., require_test_database_url(), UnsafeTestDatabaseError, validate_test_database_url() (+29 more)
...
Cohesion: 0.12
Nodes (14): ADR-006 Scientific Traceability, ADR-014: PostgreSQL/PostGIS persistence for Agroclimatic Evaluation, Consequences, Context, Decision, Source, Status, Agroclimatic Evaluation request, worker, and recovery slice (+6 more)

### Community 150 - "crop_rotation.py"
Cohesion: 0.22
Nodes (13): calculate_suitabilities(), compute_combinations(), crop_rotation(), njit, get_geotiff_extent(), ndarray, Get the spatial extent (bounding box) of a GeoTIFF file. Args: file_path (str):…, Read a GeoTIFF file with multiple bands into a NumPy array. Parameters: - fn… (+5 more)
...
### Community 154 - "environmental_information/interfaces/http.py"
Cohesion: 0.22
...
### Community 155 - "main.py"
Cohesion: 0.22
Nodes (8): health(), Host-level health endpoint., Report that the API process is ready to receive requests., create_app(), FastAPI application composition root., Build the VIA API and register its technical interfaces., FastAPI, get

### Community 157 - "execution.py"
Cohesion: 0.15
Nodes (17): Synchronous application orchestration for persisted evaluations., _to_outcome(), Domain errors for Agroclimatic Evaluation., Agroclimatic Evaluation domain layer., Agroclimatic Evaluation aggregate., CropOutcomeStatus, StrEnum, Durable per-crop outcome values owned by Agroclimatic Evaluation. (+9 more)
...
### Community 159 - "config.py"
Cohesion: 0.23
Nodes (8): _environment_float(), _environment_integer(), _optional_path(), Path, Environment-backed configuration for the VIA application host., Settings for the PostgreSQL polling worker process., WorkerSettings, test_worker_run_requires_scientific_paths()
...
Cohesion: 0.31
Nodes (10): _body(), Any, Response, End-to-end API tests for evaluation request persistence., _request(), _test_app(), test_create_get_and_list_evaluation(), scenario() (+2 more)

### Community 163 - "interpolate_precipitation"
Cohesion: 0.29
Nodes (7): Interpolates or retrieves downscaled climate data (precipitation and…, _create_interpolation_folders(), interpolate_precipitation(), interpolate_temperature(), interpolate_temperature_method(), domain: [y_max, x_min, y_min, x_max], domain: [y_max, x_min, y_min, x_max]

### Community 164 - "ADR-015: PostgreSQL polling for Agroclimatic Evaluation worker dispatch"
Cohesion: 0.33
Nodes (5): ADR-015: PostgreSQL polling for Agroclimatic Evaluation worker dispatch, Consequences, Context, Decision, Status
...
### Community 167 - "read_area_from_netcdf_list"
Cohesion: 0.50
Nodes (4): downscaled_files: list of netcdf files overlap: In Degree extent: [North, Left,…, read_area_from_netcdf_list(), read_ind_date_file(), sort_coordinatelist()
...
## Knowledge Gaps
- **174 isolated node(s):** `via-backend`, `Status`, `Context`, `Decision`, `Consequences` (+169 more)
...
- `Domain dependency rule` (5× useful, score=4.816073593)
- `Application layer` (4× useful, score=3.849937393)
- `EnvironmentalInformationService` (3× useful, score=2.909599614)
- `ParcelGeometry` (3× useful, score=2.902001583)
- `ParcelVersion` (3× useful, score=2.877865053)
- `InMemoryParcelRepository` (3× useful, score=2.876588241)
- `ParcelRepository` (3× useful, score=2.876588239)
- `Farm Management schema ownership` (2× useful, score=1.959556993)
- `CoverageGeometry` (2× useful, score=1.943460633)
- `DatasetVersion` (2× useful, score=1.937869408)
...
- **Why does `create_app()` connect `main.py` to `FarmManagementService`, `agroclimatic_evaluation/infrastructure/postgresql_repositories.py`, `test_agroclimatic_evaluation_api.py`, `coverage.py`, `test_farm_management_api.py`, `farm_management/infrastructure/postgresql_repositories.py`, `via_backend/worker.py`, `Project`, `EnvironmentalInformationService`, `InMemoryEvaluationRepository`, `Dataset`, `Parcel`, `test_environmental_information_api.py`, `Settings`, `agroclimatic_evaluation/application/__init__.py`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `EnvironmentalInformationService` connect `EnvironmentalInformationService` to `coverage.py`, `DatasetVersion`, `DomainValidationError`, `Dataset`, `test_environmental_information_coverage_postgresql.py`, `CoverageMeasurement`, `environmental_information/interfaces/http.py`, `main.py`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `AgroclimaticEvaluationService` connect `agroclimatic_evaluation/application/__init__.py` to `DomainValidationError`, `agroclimatic_evaluation/infrastructure/postgresql_repositories.py`, `Evaluation`, `InMemoryEvaluationRepository`, `test_agroclimatic_evaluation_domain.py`, `main.py`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Are the 15 inferred relationships involving `Evaluation` (e.g. with `AgroclimaticEvaluationExecutionService` and `EvaluationResult`) actually correct?**
  _`Evaluation` has 15 INFERRED edges - model-reasoned connections that need verification._
