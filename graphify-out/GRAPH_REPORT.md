...
## Corpus Check
- 198 files · ~89,373 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1296 nodes · 2425 edges · 135 communities (61 shown, 35 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 197 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d554eae0`
...
- PostgreSQL/PostGIS Farm Management persistence decision
- nc_tools.py
...
3. `DatasetVersion` - 33 edges
4. `Dataset` - 32 edges
5. `EnvironmentalInformationService` - 31 edges
6. `Project` - 28 edges
7. `create_router()` - 21 edges
8. `create_app()` - 21 edges
9. `climsuit_new()` - 20 edges
10. `DomainValidationError` - 20 edges
...
- `ADR-008 Sequential Crop Execution` --references--> `Architecture and Backend for CropSuiteLite Huaura v2`  [INFERRED]
  docs/adr/ADR-008-sequential-crop-execution.md → graphify-out/converted/Arquitectura_y_backend_CropSuiteLite_Huaura_v2_0e904345.md
...
Cohesion: 0.08
Nodes (49): include_name(), Alembic environment for VIA database migrations., Limit autogeneration to bounded-context-owned schemas., run_migrations_offline(), run_migrations_online(), Base, DeclarativeBase, SQLAlchemy metadata owned by Farm Management Infrastructure. (+41 more)
...
Cohesion: 0.09
Nodes (29): aggregate_soil_raster_lst(), calcification_map(), calculate_slope(), cropsuitability(), get_soil_data(), get_suitability_val_dict(), get_texture_class(), get_valid_dtype() (+21 more)
...
Cohesion: 0.08
Nodes (29): process_index(), find_max_sum_new(), get_id_list_start(), get_lethal_max_precipitation(), get_lethal_min_precipitation_duration(), get_photoperiod_params(), get_prec_requirements(), get_suitability_val_dict_xarray() (+21 more)
...
Cohesion: 0.10
Nodes (27): calculate_average_sunshine(), calculate_day_length(), climate_suitability(), climsuit_new(), process_index(), find_max_sum_new(), get_lethal_max_precipitation(), get_lethal_min_precipitation_duration() (+19 more)

### Community 17 - "test_environmental_coverage.py"
Cohesion: 0.13
Nodes (18): process_day_climsuit_memopt(), process_day_concfut(), Processes climate suitability data for a specific day and saves the results to…, process_tempday_interp(), get_formula(), get_id_list_start(), get_plant_param_interp_forms_dict(), print_crop_param_output() (+10 more)

### Community 18 - "FarmManagementService"
Cohesion: 0.17
...
### Community 19 - "CropSuite.py"
Cohesion: 0.13
Nodes (21): # NOTE: self.extent is modified here to align with grid, climate_suitability_xarray(), compute_suitability(), Calculates climate suitability for multiple plants based on the CropSuite…, calculate_suitabilities(), compute_combinations(), crop_rotation(), njit (+13 more)

### Community 20 - "DatasetVersion"
Cohesion: 0.30
Nodes (10): DatasetVersion, Immutable reproducibility metadata for one dataset release., DatasetVersionRecord, Database records for Environmental Information; not domain entities., PostgreSQLDatasetVersionRepository, UUID, PostgreSQL/PostGIS repositories for Environmental Information., _version_from_row() (+2 more)
...
### Community 22 - "PostgreSQL/PostGIS Farm Management persistence decision"
Cohesion: 0.25
Nodes (8): Durable Farm Management persistence, Polygon and MultiPolygon round-trip preservation, Infrastructure-only persistence mapping, PostGIS MULTIPOLYGON SRID 4326 storage, PostgreSQL/PostGIS Farm Management persistence decision, PostGIS geometry storage contract, Durable Farm Management persistence continuation query, PostgreSQL/PostGIS durable persistence workflow
...
### Community 26 - "PostgreSQLDatasetRepository"
Cohesion: 0.18
Nodes (8): Base, DeclarativeBase, SQLAlchemy metadata owned by Environmental Information Infrastructure., Declarative base for Environmental Information persistence records., DatasetRecord, _dataset_from_record(), PostgreSQLDatasetRepository, SessionFactory
...
Cohesion: 0.33
Nodes (6): Farm Management schema ownership, Projects, parcels, and parcel_versions tables, farm_management schema and owned tables, Optional durable repository adapter, Bounded-context ownership and Farm Management repositories, Farm Management ownership query
...
Cohesion: 0.39
Nodes (9): Multicrop Parcel Evaluation, run_evaluation Service, ADR-004 CropSuiteLite Port and Adapter, ADR-008 Sequential Crop Execution, CropSuiteLite isolation boundary, CropSuiteAdapter, CropSuiteLite Integration Boundary, ICropSuitabilityEngine (+1 more)

### Community 31 - "test_environmental_information_postgresql.py"
Cohesion: 0.33
...
### Community 37 - "test_environmental_information_domain.py"
Cohesion: 0.36
...
### Community 38 - "Farm Management persistence"
Cohesion: 0.29
Nodes (7): Database configuration and Alembic migrations, Farm Management persistence, Process-local in-memory repository adapter, Local database and migrations, Open decisions, Open persistence deployment decisions, Ownership and schema

### Community 39 - "VIA architecture implementation roadmap"
Cohesion: 0.20
Nodes (11): Project, Parcel, and ParcelVersion domain model, Architecture source and PoC preservation, Cross-stage architecture gates, Explanation and comparison increment, Modeling increment, Parcel and environmental coverage increment, Public deployment increment, Result querying increment (+3 more)
...
Cohesion: 0.13
Nodes (14): create_database(), Engine, SessionFactory, Host-level SQLAlchemy engine and session construction., Create the shared engine and short-lived session factory., Shared technical infrastructure used by the application composition root., health(), Host-level health endpoint. (+6 more)
...
Cohesion: 0.18
Nodes (15): DomainValidationError, ValueError, Domain errors raised by Environmental Information invariants., Raised when environmental metadata violates a domain invariant., Environmental Information domain layer., Environmental Information domain model., _validate_text(), Framework-neutral spatial metadata value objects. (+7 more)

### Community 75 - "DatasetVersionConflictError"
Cohesion: 0.15
Nodes (11): DatasetVersionConflictError, RuntimeError, Raised when a dataset version identifier has already been registered., Environmental Information infrastructure layer., InMemoryDatasetRepository, InMemoryDatasetVersionRepository, UUID, In-memory Environmental Information repository adapters. (+3 more)
...
## Knowledge Gaps
- **122 isolated node(s):** `via-backend`, `Status`, `Context`, `Decision`, `Consequences` (+117 more)
...
- `Domain dependency rule` (3× useful, score=2.97981797)
- `Application layer` (3× useful, score=2.97981797)
- `ParcelVersion` (3× useful, score=2.979817959)
- `InMemoryParcelRepository` (3× useful, score=2.978495914)
- `ParcelRepository` (3× useful, score=2.978495912)
- `ParcelVersionRecord` (2× useful, score=1.987641657)
- `PostgreSQLParcelRepository` (2× useful, score=1.987641657)
- `ProjectRepository` (2× useful, score=1.984997555)
...
- **Why does `EnvironmentalInformationService` connect `Dataset` to `DomainValidationError`, `DatasetVersionConflictError`, `DatasetVersion`, `main.py`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `create_app()` connect `main.py` to `farm_management/application/service.py`, `farm_management/infrastructure/postgresql_repositories.py`, `Dataset`, `DatasetVersionConflictError`, `Parcel`, `Settings`, `FarmManagementService`, `DatasetVersion`, `PostgreSQLDatasetRepository`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
...
- **Are the 9 inferred relationships involving `DatasetVersion` (e.g. with `DatasetVersionResult` and `EnvironmentalInformationService`) actually correct?**
  _`DatasetVersion` has 9 INFERRED edges - model-reasoned connections that need verification._