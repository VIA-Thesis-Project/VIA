...
## Corpus Check
- 242 files · ~104,349 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1786 nodes · 3778 edges · 157 communities (84 shown, 31 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 363 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b4df7441`
...
10. `DomainValidationError` - 26 edges
...
Cohesion: 0.16
Nodes (22): CheckDatasetVersionCoverage, CoverageClassification, CoverageMeasurement, StrEnum, Extent-based relationship between a dataset version and a parcel., Successful, CRS-aware area measurement returned by a spatial port., _multi_polygon(), _polygon() (+14 more)

### Community 19 - "cropsuite_adapter.py"
Cohesion: 0.08
Nodes (58): CropExecutionStatus, CropSuitabilityEngineError, CropSuitabilityExecutionError, InvalidEngineOutputError, RuntimeError, StrEnum, Scientific outcomes reported independently of Evaluation lifecycle state., Current PoC parcel summary for the crop-suitability output. (+50 more)
...
Cohesion: 0.11
Nodes (30): ExecuteEvaluation, Request synchronous execution of one already-persisted evaluation., AgroclimaticEvaluationExecutionService, Exception, Execute requested crops sequentially through the Application-owned port., CropSuitabilityRequest, CropSuitabilityResult, ICropSuitabilityEngine (+22 more)
...
Cohesion: 0.08
Nodes (31): Synchronous application orchestration for persisted evaluations., _to_outcome(), Transport-neutral Agroclimatic Evaluation results., EvaluationConflictError, InvalidEvaluationTransitionError, RuntimeError, Domain errors for Agroclimatic Evaluation., Raised when an Evaluation lifecycle transition is not allowed. (+23 more)

### Community 38 - "main.py"
Cohesion: 0.16
Nodes (10): Environmental Information interface layer., create_database(), Engine, SessionFactory, Host-level SQLAlchemy engine and session construction., Create the shared engine and short-lived session factory., Shared technical infrastructure used by the application composition root., create_app() (+2 more)
...
Cohesion: 0.10
Nodes (28): Application-owned boundary for one crop suitability evaluation., DomainValidationError, ValueError, Raised when evaluation data violates a domain invariant., Current reproducibility trace without engine-specific filesystem details., ScientificTrace, ParcelSnapshot, _parse_multi_polygon() (+20 more)

### Community 65 - "agroclimatic_evaluation/infrastructure/postgresql_repositories.py"
Cohesion: 0.14
Nodes (21): Base, DeclarativeBase, SQLAlchemy metadata owned by Agroclimatic Evaluation Infrastructure., Declarative base for Agroclimatic Evaluation persistence records., Agroclimatic Evaluation infrastructure layer., CropOutcomeRecord, EvaluationCropRecord, EvaluationRecord (+13 more)

### Community 67 - "DomainValidationError"
Cohesion: 0.08
Nodes (37): InvalidSpatialInputError, CoverageComputation, RuntimeError, ValueError, Application ports for Environmental Information spatial collaboration., Raised when supplied parcel geometry is not topologically usable., Raised when the configured spatial implementation cannot execute., SpatialCoverageUnavailableError (+29 more)
...
### Community 74 - "test_environmental_information_domain.py"
Cohesion: 0.36
...
Cohesion: 0.09
Nodes (38): DatasetVersion, Immutable reproducibility metadata for one dataset release., Positive horizontal and vertical source-cell resolution., A rectangular extent expressed in the dataset version's CRS., SpatialExtent, SpatialResolution, Base, DeclarativeBase (+30 more)
...
Cohesion: 0.26
Nodes (18): clean_evaluations(), database(), _database_url(), _evaluation(), _multipolygon(), _polygon(), Engine, fixture (+10 more)
...
Cohesion: 0.12
Nodes (14): ADR-006 Scientific Traceability, ADR-014: PostgreSQL/PostGIS persistence for Agroclimatic Evaluation, Consequences, Context, Decision, Source, Status, Agroclimatic Evaluation request and synchronous execution slice (+6 more)
...
- **Why does `create_app()` connect `main.py` to `FarmManagementService`, `agroclimatic_evaluation/infrastructure/postgresql_repositories.py`, `DomainValidationError`, `Evaluation`, `farm_management/infrastructure/postgresql_repositories.py`, `Dataset`, `Project`, `EnvironmentalInformationService`, `DatasetVersion`, `Parcel`, `Settings`, `CoverageMeasurement`, `agroclimatic_evaluation/application/__init__.py`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `EnvironmentalInformationService` connect `EnvironmentalInformationService` to `DomainValidationError`, `main.py`, `Dataset`, `DatasetVersion`, `test_environmental_information_coverage_postgresql.py`, `CoverageMeasurement`, `environmental_information/interfaces/http.py`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `AgroclimaticEvaluationService` connect `agroclimatic_evaluation/application/__init__.py` to `DomainValidationError`, `Evaluation`, `main.py`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Are the 14 inferred relationships involving `Evaluation` (e.g. with `AgroclimaticEvaluationExecutionService` and `EvaluationResult`) actually correct?**
  _`Evaluation` has 14 INFERRED edges - model-reasoned connections that need verification._
