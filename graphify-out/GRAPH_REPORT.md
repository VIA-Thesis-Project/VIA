...
## Corpus Check
- 314 files · ~158,883 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2975 nodes · 7414 edges · 202 communities (118 shown, 31 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 923 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b84759ea`
...
1. `Evaluation` - 94 edges
2. `DomainValidationError` - 66 edges
3. `WaterRegime` - 56 edges
4. `PostgreSQLEvaluationRepository` - 53 edges
5. `EvaluationStatus` - 49 edges
6. `EnvironmentalInformationService` - 49 edges
7. `InMemoryEvaluationRepository` - 45 edges
8. `DatasetVersion` - 45 edges
9. `AgroclimaticEvaluationService` - 44 edges
...
Cohesion: 0.06
Nodes (99): include_name(), Alembic environment for VIA database migrations., Limit autogeneration to bounded-context-owned schemas., run_migrations_offline(), run_migrations_online(), EnvironmentalInputReference, Caller-selected exact environmental dataset version., Base (+91 more)
...
Cohesion: 0.26
Nodes (13): AgroclimaticEvaluationWorker, Discover queued IDs and delegate all execution semantics to Application., InMemoryEvaluationRepository, _engine_result(), _executor(), FakeEngine, test_claim_conflict_is_benign_and_later_work_continues(), test_no_queued_work_never_calls_engine() (+5 more)
...
Cohesion: 0.08
Nodes (55): CropSuiteComparisonAdapter, _is_within(), Path, Compare persisted CropSuiteLite rasters on common valid support., _file_identity(), FilesystemScientificArtifactStore, _is_within(), PublishedScientificArtifact (+47 more)
...
Cohesion: 0.10
Nodes (13): Exception, Validate deterministic scientific ranking semantics., validate_comparable_crops(), InvalidEvaluationTransitionError, Raised when an Evaluation lifecycle transition is not allowed., Evaluation, EvaluationScenarioResult, WaterRegime (+5 more)
...
Cohesion: 0.05
Nodes (74): EnvironmentalInputResolutionError, RuntimeError, Raised when an exact caller-selected environmental version cannot be resolved., IEnvironmentalInputIntegrityVerifier, Verify resolved environmental provenance against scientific source fingerprints., EnvironmentalInputManifest, EnvironmentalInputSnapshot, Immutable set of exact environmental inputs resolved for an evaluation. (+66 more)
...
Cohesion: 0.08
Nodes (44): _availability(), CommonSupportReadResult, ComparableCropReadResult, CropEvidenceResult, EvaluationEvidenceResult, EvaluationReadResult, EvaluationResultAvailability, PersistedCropOutcomeResult (+36 more)
...
### Community 45 - "ScientificTrace"
Cohesion: 0.21
Nodes (14): Current reproducibility trace with opaque engine source evidence., The reliable parcel summary currently returned by the scientific boundary., ScientificTrace, SuitabilitySummary, _outcome_from_record(), _evaluation(), _outcome(), parametrize (+6 more)
...
Cohesion: 0.22
Nodes (16): FinalizedCommonSupport, FinalizedCommonSupportStatus, FinalizedComparableCrop, FinalizedEvaluationResult, GetFinalizedEvaluationResult, _common_support(), _evaluate(), _finalized_result() (+8 more)

### Community 67 - "DatasetVersion"
Cohesion: 0.09
Nodes (23): Application ports for Environmental Information spatial collaboration., Domain errors raised by Environmental Information invariants., Environmental Information domain layer., Dataset, DatasetVersion, Environmental Information domain model., Stable logical identity for a geoenvironmental dataset., Immutable reproducibility metadata for one dataset release. (+15 more)
...
Cohesion: 0.06
Nodes (62): CreateDataset, CreateDatasetVersion, Commands expressing Environmental Information use-case intent., Environmental Information application layer., Protocol, Measure an external geometry against a registered dataset extent., SpatialCoveragePort, Stable contracts deliberately published to other bounded contexts. (+54 more)
...
### Community 75 - "via_backend/worker.py"
Cohesion: 0.05
Nodes (48): Side-effect-free FastAPI application composition., InvalidSpatialInputError, RuntimeError, ValueError, Raised when supplied parcel geometry is not topologically usable., Raised when the configured spatial implementation cannot execute., SpatialCoverageUnavailableError, DatasetVersionConflictError (+40 more)
...
### Community 137 - "agroclimatic_evaluation/application/service.py"
Cohesion: 0.09
Nodes (30): Commands expressing Agroclimatic Evaluation use-case intent., FinalizedCropOutcome, FinalizedCropOutcomeStatus, FinalizedScientificTrace, FinalizedSuitabilitySummary, StrEnum, Stable contracts deliberately published to other bounded contexts., Published water-regime values shared with consumer bounded contexts. (+22 more)
...
### Community 150 - "EvaluationConflictError"
Cohesion: 0.18
Nodes (6): EvaluationConflictError, RuntimeError, Raised when an evaluation identity already exists., UUID, In-memory Agroclimatic Evaluation repository adapter., _validate_limit()
...
### Community 160 - "IDefaultViabilityPolicyProvider"
Cohesion: 0.29
...
Cohesion: 0.07
Nodes (39): _is_finite_number(), datetime, Immutable environmental input snapshots owned by Agroclimatic Evaluation., _validate_aware_datetime(), _validate_crs(), _validate_positive_number(), _validate_text(), DomainValidationError (+31 more)

### Community 162 - "test_agroclimatic_evaluation_api.py"
Cohesion: 0.18
Nodes (31): _app_with(), _body(), _completed_outcomes(), _evaluation(), _evaluation_with_comparison(), _outcome(), Any, FastAPI (+23 more)

### Community 163 - "cropsuite_adapter.py"
Cohesion: 0.07
Nodes (81): CropExecutionStatus, CropSuitabilityExecutionError, InvalidEngineOutputError, Scientific outcomes reported independently of Evaluation lifecycle state., Raised when the existing engine service cannot produce a report., Raised when the engine report does not satisfy the expected PoC contract., Opaque scientific source identity and its engine-reported SHA-256., Current PoC parcel summary for the crop-suitability output. (+73 more)
...
Cohesion: 0.06
Nodes (65): _comparison_request(), datetime, Synchronous application orchestration for persisted evaluations., _to_common_support(), _to_comparable_crop(), _to_outcome(), Agroclimatic Evaluation application layer., CommonSupportResult (+57 more)
...
Cohesion: 0.08
Nodes (39): EnvironmentalInputReferenceInput, ParcelSnapshotInput, Transport-neutral parcel state supplied by an authorized caller., RequestEvaluation, GetEvaluation, GetEvaluationEvidence, GetEvaluationResult, ListEvaluations (+31 more)
...
Cohesion: 0.18
Nodes (20): Fail one operator-confirmed orphaned active evaluation., RecoverEvaluation, AgroclimaticEvaluationRecoveryService, Logger, Mark an operator-confirmed active orphan as failed without retrying it., UUID, Run explicit fail-only orphan recovery without composing CropSuiteLite., recover_evaluation() (+12 more)
...
Cohesion: 0.14
Nodes (19): _artifact(), _evaluation(), FakeComparisonEngine, datetime, parametrize, Path, UUID, WaterRegime (+11 more)
...
- **Why does `create_app()` connect `create_app` to `FarmManagementService`, `test_agroclimatic_evaluation_api.py`, `DatasetVersion`, `Project`, `farm_management/infrastructure/postgresql_repositories.py`, `Parcel`, `EnvironmentalInformationService`, `via_backend/worker.py`, `agroclimatic_evaluation/infrastructure/postgresql_repositories.py`, `InMemoryEvaluationRepository`, `AgroclimaticEvaluationService`, `test_farm_management_api.py`, `test_environmental_information_api.py`, `Settings`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `FarmManagementService` connect `FarmManagementService` to `Project`, `DomainValidationError`, `Parcel`, `via_backend/worker.py`, `create_app`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `AgroclimaticEvaluationService` connect `AgroclimaticEvaluationService` to `FinalizedCommonSupportStatus`, `DomainValidationError`, `test_agroclimatic_evaluation_api.py`, `Evaluation`, `agroclimatic_evaluation/application/service.py`, `WaterRegime`, `via_backend/worker.py`, `agroclimatic_evaluation/infrastructure/postgresql_repositories.py`, `ScientificTrace`, `agroclimatic_evaluation/application/__init__.py`, `EvaluationConflictError`, `create_app`?**
...
- **Are the 25 inferred relationships involving `DomainValidationError` (e.g. with `AgroclimaticEvaluationRecoveryService` and `AgroclimaticEvaluationService`) actually correct?**
  _`DomainValidationError` has 25 INFERRED edges - model-reasoned connections that need verification._
- **Are the 33 inferred relationships involving `WaterRegime` (e.g. with `RequestEvaluation` and `_comparison_request()`) actually correct?**
  _`WaterRegime` has 33 INFERRED edges - model-reasoned connections that need verification._