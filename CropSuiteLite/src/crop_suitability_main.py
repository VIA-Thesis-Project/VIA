...
from .spatial_validity import resample_valid, covered_gradient
...
def aggregate_soil_raster_lst(file_list, domain, final_shape, weighting_method = 0, conversion_factor = 1., weighting = [2., 1.5, 1., 0.75, 0.5, 0.25]):
    ...
    if weighting_method == 0:
        ...
        # Empty coverage remains missing without a mean-of-empty-slice warning.
        layers = np.asarray(layers)
        count = np.sum(np.isfinite(layers), axis=0)
        mean = np.full(layers.shape[1:], np.nan, dtype=float)
        np.divide(np.nansum(layers, axis=0), count, out=mean, where=count > 0)
...
def calculate_slope(dem_path, output_shape, extent):
    if output_shape == (0, 0) or extent == [0, 0, 0, 0]:
        with rasterio.open(dem_path, 'r') as dem:
            ...
            dem_array = dem.read(1, masked=True).astype(float).filled(np.nan)
    else:
        cell_size = ((extent[2] - extent[0]) / output_shape[1])
        domain = [extent[1], extent[0], extent[3], extent[2]]
        layers, nodata = load_specified_lines(dem_path, domain)
        dem_array = layers[0]
        valid = np.isfinite(dem_array)
        if nodata is not None:
            valid &= dem_array != nodata
        dem_array = resample_valid(dem_array, output_shape, valid)
    R = 6371000
    resolution_rad = np.deg2rad(cell_size)
    resolution_m = R * resolution_rad
    # Do not differentiate through nodata; use one-sided differences at the
    # coverage boundary instead of losing another ring of valid DEM cells.
    dz_dy, y_neighbour = covered_gradient(dem_array, resolution_m, axis=0)
    dz_dx, x_neighbour = covered_gradient(dem_array, resolution_m, axis=1)
    slope = np.arctan(np.sqrt(dz_dx**2 + dz_dy**2))
    slope_degrees = np.rad2deg(slope)
    slope_degrees[~(x_neighbour | y_neighbour)] = np.nan
...
def cropsuitability(config, clim_suit_shape, plant_formulas, plant_params, extent, land_sea_mask, results_path, multiple_cropping_sum=None):
    ...
    for plant_idx, plant in enumerate([p for p in os.listdir(results_path) if os.path.isdir(os.path.join(results_path, p))]):
        ...
        soil_suitablility[np.asarray(land_sea_mask) != 1] = -1
        ...
        suitability[np.asarray(land_sea_mask) != 1] = -1
        ...
        suitability_multi[suitability == -1] = -1
