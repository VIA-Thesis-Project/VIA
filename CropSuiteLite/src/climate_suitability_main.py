...
from .spatial_validity import climate_coverage
...
def process_day_climsuit_memopt(args):
    ...
    vern_period = 150
    water_mask = climate_coverage(temperature, precipitation, water_mask, irrigation == 1)
    # Numerical placeholders exist only outside the evaluation mask. All
    # outputs for these cells are explicitly nodata, never suitability zero.
    temperature = np.where(water_mask[..., None], temperature, 0)
    precipitation = np.where(water_mask[..., None], precipitation, 0)
    ...
    if wintercrop:
        ...
        vern_prec[np.sum(prec_to_vern[..., :sowprec_params[1]], axis=2, dtype=np.int16) < sowprec_params[0] * 10] = 0
    ...
    temp = np.full((temperature.shape[0], temperature.shape[1]), -1, dtype=np.int16)
    ...
    if irrigation == 0:
        prec = np.full((temperature.shape[0], temperature.shape[1]), -1, dtype=np.int32)
        ...
        # Downscaling stores daily precipitation as mm * 10. The accumulated
        # value is still in tenths of mm; prec_vals membership knots are in mm.
        prec_mm = prec[water_mask == 1] / 10.0
        prec[water_mask == 1] = (get_suitability_val_dict(plant_params_forumlas, 'prec', prec_mm) * 100).astype(np.int8)
        prec = prec.astype(np.int8)
    else:
        prec = np.zeros_like(temp)

    if crop_failures.ndim > 1:
        failure_suit = np.full((temperature.shape[0], temperature.shape[1]), -1, dtype=np.int8)
    ...
    if irrigation == 0 and growing_cycle < 365 and not wintercrop:
        # sowprec_params: [prec_req_after_sow, prec_req_days]
        prec_after_sowing = np.zeros((temperature.shape[0], temperature.shape[1]), dtype=np.uint16)
        prec_after_sowing[water_mask == 1] = np.sum(precipitation[water_mask == 1, :sowprec_params[1]], axis=1, dtype=np.uint16)
        prec[(water_mask == 1) & (prec_after_sowing < sowprec_params[0]*10)] = 0
    ...
    for factor in (temp, prec, failure_suit, sunshine_hours):
        factor[~water_mask] = -1
...
def climsuit_new(climate_config, extent, temperature, precipitation, land_sea_mask, plant_params, results_path, plant, area_name, day_interval = 1) -> list:
    ...
    water_mask = climate_coverage(temperature, precipitation, land_sea_mask, irrigation)
    land_sea_mask = water_mask.astype(np.uint8)
