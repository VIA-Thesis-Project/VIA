...
def process_day_climsuit_xarray(day, temperature_cycle, precipitation_cycle,
    ...
    coverage = (np.isfinite(temperature_cycle.data) & (temperature_cycle.data != -32767)).all(dim='time')
    if is_irrigated == 0:
        coverage &= (np.isfinite(precipitation_cycle.data) & (precipitation_cycle.data >= 0)).all(dim='time')
    ...
    mask_layer = mask_layer & coverage
    temperature_cycle = temperature_cycle.where(mask_layer, 0)
    precipitation_cycle = precipitation_cycle.where(mask_layer, 0)

    temp = temperature_cycle.mean(dim='time')
    temp = temp.where(mask_layer, 0)
    ...
    if is_irrigated == 0:
        ...
        prec = prec.where(mask_layer, 0)
        if wintercrop:
            # TODO IMPLEMENT
            #prec = ((prec * len_growing_cycle) + (vern_prec * vernalization_params[5])) / (len_growing_cycle + vernalization_params[5])
            pass
        # Downscaled sums are in tenths of mm; membership prec_vals are in mm.
        prec_ = get_suitability_val_dict_xarray(prec.data / 10.0,  plant_params[plant], config, 'prec')*100
        prec_ = prec_.where(mask_layer, -1)
    ...
    if is_irrigated == 0 and len_growing_cycle < 365 and not wintercrop:
        ...
        sowing_day_prec_cond = prec_after_sowing < sowprec_params[0]*10
    ...
    alldata = xarray.merge([temp_, prec_, failure_suit, sunshine_hours]).where(mask_layer, -1).load()
...
def compute_suitability(climate_config, results_path, plant_params, plant, land_sea_mask, extent, tmp_folder: str = 'temp2'):
    ...
    # Daily nodata must also mask final summaries and limiting factors.
    water_mask &= np.all(fuzzy_temp_growing_cycle_wdoy >= 0, axis=-1)
    water_mask &= np.all(fuzzy_precip_growing_cycle_wdoy >= 0, axis=-1)
