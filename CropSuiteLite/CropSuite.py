    ...
    def compute_climate_suitability(self, 
        ...
        if all(os.path.exists(clims) for clims in climsuits):
            ...
            if doit_withxarray:
                ...
                temperature = read_area_from_netcdf_list(temp_files, overlap=False, extent=extent, dayslices=temp_dailyfiles, workers=int(self.climate_config['options'].get('max_workers', 1)))
                precipitation = read_area_from_netcdf_list(prec_files, overlap=False, extent=extent, dayslices=prec_dailyfiles, workers=int(self.climate_config['options'].get('max_workers', 1)))
