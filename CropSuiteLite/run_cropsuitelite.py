...
def main(config_path:str):
    ...
    # A reviewed configuration can be run without regenerating maize.inf or
    # deriving another output directory from the climate scenario name.
    run_config = config_dict['GENERAL'].get('run_config')
    if run_config:
        run_config = os.path.join(os.path.dirname(os.path.abspath(__file__)), run_config)
        cs_lite = CropSuiteLite(config_file=run_config)
        print(f'-------> Running {run_config}')
        cs_lite.run()
        return
